...
!> This subroutine allocates memory for vertices
	...
	ALLOCATE(SUMVARS(NOF_VARIABLES),MAXVARS(NOF_VARIABLES),aver_vars(nof_Variables))
	ALLOCATe(CORDS(3))
	...
	ALLOCATE(LEFTV(NOF_VARIABLES))
	ALLOCATE(RIGHTV(NOF_VARIABLES))
    ALLOCATE(SUMVARS(NOF_VARIABLES),MAXVARS(NOF_VARIABLES)) 
	ALLOCATe(CORDS(2))
...
do i=1,kmaxe
 allocate(INTEG_BASIS_dg(i)%value(idegfree));INTEG_BASIS_dg(i)%value(:)=zero
end do
end if
do i=1,kmaxe
...
ALLOCATE(totalmm(1:NUM_DG_DOFS,1:NUM_DG_DOFS),INVmm(1:NUM_DG_DOFS,1:NUM_DG_DOFS),weights_dg(1:qp_quad))
ALLOCATE(FLUX_TERM_Z(1:NOF_VARIABLES),FLUX_TERM_Y(1:NOF_VARIABLES),FLUX_TERM_X(1:NOF_VARIABLES))
allocate(basis_vector(1:idegfree+1),tempsol(1,1:idegfree+1))

end if

! if (DG.EQ.1)THEN
! ALLOCATE(LSCQM_DG(1:IDEGFREE+1,1:IDEGFREE+1),QFF_DG(1:IDEGFREE+1,1:IDEGFREE+1),RFF_DG(1:IDEGFREE+1,1:IDEGFREE+1),QTFF_DG(1:IDEGFREE+1,1:IDEGFREE+1)&
! ,INVRFF_DG(1:IDEGFREE+1,1:IDEGFREE+1))
! 
! 
! END IF
